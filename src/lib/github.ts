...
export async function getUpdate(date: string): Promise<UpdateData | null> {
  const url = `${API}/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/${UPDATES_PATH}/${date}.md`;
...
export async function saveUpdate(date: string, data: UpdateData): Promise<void> {
  const url = `${API}/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/${UPDATES_PATH}/${date}.md`;
  ...
  const body: Record<string, string> = { message: `update: ${date}`, content };
  if (sha) body.sha = sha;
...
export async function listUpdates(): Promise<{ date: string; data: UpdateData }[]> {
  const url = `${API}/repos/${GITHUB_OWNER}/${GITHUB_REPO}/contents/${UPDATES_PATH}`;
  const res = await fetch(url, { headers: headers(), cache: "no-store" });
